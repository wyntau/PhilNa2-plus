...
<div id="respond" class="box icon content">
  ...
  <form id="commentform" action="<?php echo site_url('/wp-comments-post.php'); ?>" method="post" class="clearfix">
    <div id="commentbox" class="left">
      ...
      <span id="comtinfo">说点什么再走吧!</span><i id="num">0</i>
    </div>
...
?>
        <div id="welcome_info clearfix">
          ...
          <p>
            <?php _e('Logged in as', YHL); ?>
            <a href="<?php echo site_url('/wp-admin/profile.php'); ?>">
              <strong><?php echo $user_identity; ?></strong>
            </a>.
            <a href="<?php echo wp_logout_url( get_permalink() ); ?>" title="<?php _e('Log out of this account', YHL); ?>">
              <?php _e('Logout &raquo;', YHL); ?>
            </a>
          </p>
...
?>
  ...
  <strong class="comment_notice">NOTICE:</strong> You should type some Chinese word (like “你好”) in your comment to pass the spam-check, thanks for your patience!
</div>
<?php
/* philna hook */
do_action('commentAfter');
if(is_404()){
?>
